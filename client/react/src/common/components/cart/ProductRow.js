...
class ProductRow extends React.Component {
  ...
  render() {
    const { product: {id, name, company, description, quantity, price, value, onHand}, removeProduct } = this.props;
    return (
      ...
      <tr key={id}>
        ...
        <td>{company}</td>
        <td>{description}</td>
        <td><input type="number" value={quantity} onChange={this.onQuantityChange} min="0" max={onHand} /></td>
        <td><input type="number" value={price} onChange={this.onPriceChange} min="0" /></td>
        <td><input type="number" value={value || price * quantity} onChange={this.onValueChange} min="0" /></td>
        <td>
